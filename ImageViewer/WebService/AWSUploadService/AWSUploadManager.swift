...
final class AWSUploadManager: NSObject {
    ...
    /// Configure AWS keys
    func configure() {
        let credentialsProvider: AWSStaticCredentialsProvider = AWSStaticCredentialsProvider(accessKey: AWSUploadConfigration.accessKeyID.rawValue, secretKey: AWSUploadConfigration.secretKey.rawValue)
    ...
    func uploadImage(in folder: AWSBucketFolder,
                    ...
                    if let deleteImageName = deleteImageName {
                        ...
                        self.deleteData(in: folder, with: deleteImageName) { (result) in
                            ...
                            case .success((_, let index)):
                                let uploadedImagePath = AWSUploadConfigration.basePath.rawValue + remotePath
        ...
        func uploadImage(with data: Data) {
            ...
            transferUtility.uploadData(
                data,
                bucket : AWSUploadConfigration.bucket.rawValue,
                key: remotePath,
...
extension AWSUploadManager {
    ...
    func uploadVideo(
        ...
        func uploadFinalVideo(with url: URL, isRemoveLoader: Bool, completion: @escaping Handler) {
            ...
            self.multipartCompletionHandler = { (task, error) -> Void in
                ...
                DispatchQueue.main.async(execute: {
                    ...
                    if let error = error {
                        ...
                        let finalURL = AWSUploadConfigration.basePath.rawValue + videoname
            ...
            AWSS3TransferUtility.default().uploadUsingMultiPart(fileURL: url, bucket: AWSUploadConfigration.bucket.rawValue, key: videoname, contentType: mimeType.rawValue, expression: expression, completionHandler: self.multipartCompletionHandler).continueWith() { (task) -> Any? in
...
extension AWSUploadManager {
    ...
    func deleteData(in folder: AWSBucketFolder,
        ...
        deleteObjectRequest?.bucket = AWSUploadConfigration.bucket.rawValue
                ...
                if let _ = task.result {
                    ...
                    if let deleteThumbImage = deleteThumbImage, let deleteThumbFolder = deleteThumbFolder {
                        ...
                        self.deleteData(in: deleteThumbFolder, with: deleteThumbImage) { (result) in
                            switch result {
                            case .success((_, let index)):
                                let deleteImagePath = AWSUploadConfigration.basePath.rawValue + folder.path + name
