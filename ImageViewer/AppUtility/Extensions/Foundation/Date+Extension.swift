...
extension Date {
    ...
    func convertDateFormat(output format: DateFormatter, type: DateConvertionType) -> (String, Date) {
        if type == .utc {
            let utcDate = self.convertToUTC()
            return (format.string(from: utcDate), utcDate)
            
        } else if type == .local {
            let localDate = self.convertToLocal()
            return (format.string(from: localDate), localDate)
            
        } else {
            return (format.string(from: self), self)
        }
    }
